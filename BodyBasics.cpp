...
double BdyLength(Joint p1, Joint p2)
{
	return (sqrt(
...
void RecBody(Joint *joints)
{
	...
	double dLeftLen = BdyLength(joints[JointType_KneeLeft], joints[JointType_AnkleLeft]);
	double dRightLen = BdyLength(joints[JointType_KneeRight], joints[JointType_AnkleRight]);

	if (dLeftLen == 0 || dRightLen == 0)
		return;

	ratio = dLeftLen / dRightLen;

	dAvgRatio = ((num / (num + 1.0L))*dAvgRatio) + (ratio / (num + 1));
	num++;
	
	fprintf(fp, "dLeftLen: %f\n", dLeftLen);
	fprintf(fp, "dRightLen: %f\n", dRightLen);
	fprintf(fp, "Ratio: %f\n", ratio);
	fprintf(fp, "AvgRatio: %f over %u\n\n", dAvgRatio, num);
	fclose(fp);
...
void CBodyBasics::ProcessBody(INT64 nTime, int nBodyCount, IBody** ppBodies)
{
	if (m_hWnd)
	{
		...
		StringCchPrintf(szStatusMessage, _countof(szStatusMessage), L"  cur = %0.4f\tavg = %0.4f\tnum = %u\t\tShould see a doctor: %s", ratio, dAvgRatio, num, ((((dAvgRatio - 1.03f) > 0.096f) || ((dAvgRatio - 1.03f) < -0.096f)) && num > 48) ? L"Yes" : L"No");
